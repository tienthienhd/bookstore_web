                ...
                <div class="card-box">
                    ...
                    <div class="card-body row">
                        ...
                        <div class="col-lg-12 p-t-20 text-center"> 
                            <a class="mdl-button m-b-10 m-r-20 btn-primary" href="{{route('manager.book.edit', ['book' => $book])}}" style="color: #ffffff">{{__('btn.edit')}}</a>
                            <form action="{{route('manager.book.update.off-state', ['book' => $book])}}" method="post">
                                @csrf
                                @method('put')
                                
                                @if($book->state < 0)
                                    <input  class="mdl-button m-b-10 m-r-20 btn-circle btn-danger" type="submit" name="stopSaleBook" value="{{__('btn.stop-sale')}}" disabled="disabled">
                                    <input class="mdl-button m-b-10 m-r-20 btn-circle btn-success" type="submit" name="reRtopSaleBook" value="{{__('btn.sale-again')}}">
                                @else
                                    <input class="mdl-button m-b-10 m-r-20 btn-circle btn-danger" type="submit" name="stopSaleBook" value="{{__('btn.stop-sale')}}">
                                @endif
                            </form>
                           </div>
                    </div>
